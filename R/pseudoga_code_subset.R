...
#' @param repl Number of independent pseudotime estimates used to calculate final trajectory. Default value is 3. Increasing the value of this parameter will in general improve the accuracy of the estimate.
...
pseudoga_parallel<-function(sce,type=c("counts","nomralized","expression"),ntest=50,repl=3,subsample=100,minit=30,epsilon=0.0001,normalization=c("default","TMM","quant","cpm"),nnprop=0.3)
{
  library(SingleCellExperiment)
  type<-type[1]
...
findorders_counts_parallel<-function(data,ntest,repl,subsample,minit,epsilon,nnprop)
{
  ...
  data<-t(data)
	...
	for(i in 1:repl)
	{
	  subset<-sample(n,min(n,subsample))
	  subset1[[i]]<-subset
	  data2[[i]]<-data[subset,]
	}
